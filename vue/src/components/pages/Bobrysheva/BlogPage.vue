    ...
    <div :class="{ 'light-theme': isLightTheme, 'dark-theme': !isLightTheme }">
      ...
      <div class="main-info">
        ...
        <div class="main-cards">
          <MainCard v-for="(card, index) in filteredCards" :key="index" :card="card"></MainCard>
        </div>
...
export default {
  ...
  watch: {
    // Следим за изменением isLightTheme
    isLightTheme(newValue) {
      // Выполняем действия при изменении темы
      if (newValue) {
        // Если выбрана светлая тема
        document.body.classList.remove('dark-theme');
        // Удаляем класс dark-theme у body
      } else {
...
<style lang="less">
  ...
  .dark-theme {
    ...
    .fas.fa-bars {
      color: #ffffff; /* Установите желаемый цвет для иконки в темной теме */
    }

    .main-card .hashtags-wrapper .card-hashtag {
      color: #eaeaea;
    }
  ...
  .logo{
  font-size: 25px;
  font-family: "Cina-Medium", sans-serif;
    display: flex;
  }
  ...
  body h1 {
    margin-top: 40px;
    font-family: "Cina-SeBold", sans-serif;
    font-size: 40px;
    text-align: left;
    color: #000000;
    padding-left: 130px;
  ...
  body {
    margin: 0;
    padding: 0;
    font-family: "Cina-Regular", sans-serif;
    font-weight: lighter;
  }
  ...
  footer {
    ...
    border-radius: 20px;
    margin: 0 20px;
    font-family: "Cina-ExLight", sans-serif;
    margin-bottom: 15px;
  }
